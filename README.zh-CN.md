...
基于 LLM（兼容 OpenAI API）进行语义理解，结合向量搜索做相关性排序。**100% 客户端运行——你的书签数据不会离开浏览器**，仅将你的查询 + Top-20 候选书签发送给你配置的 LLM API。
...
- **三种界面模式**：
  ...
  - **Dashboard 工作台** — 全功能工作区：书签树 | 对话 | 搜索结果 | 智能整理 | 设置
- **深色模式** — 自动跟随系统偏好
- **中英文界面** — 跟随系统语言，可在设置中切换
- **对话历史** — 多轮搜索会话
- **自动索引** — 安装后自动索引全部书签，之后增量同步
...
│   ├── background/           # 核心逻辑
│   │   ├── search-engine.ts  # 搜索流程（嵌入 → 向量搜索 → LLM）
...
│   └── shared/               # 共享类型、工具函数、i18n、数据库、整理引擎
...
- **书签数据不离开浏览器**，仅查询和 Top-20 候选书签发送给你配置的 LLM API
